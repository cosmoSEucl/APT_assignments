...
def run_analysis(start_date='1932-01-01', end_date=None, num_factors=3, interactive=True):
    ...
    # ** NEW PLOTTING FUNCTION CALL **
    if num_factors == 1:
        plot_expected_returns_vs_beta(mean_returns, betas_matrix[:, 0], cs_with_const, model_name)
...
def plot_expected_returns_vs_beta(mean_returns, betas, cs_with_const_results, model_name):
    """
    Plots mean excess returns vs. beta, along with the cross-sectional regression line.
    (Only for the CAPM case with a single factor)
    """
    
    # Reshape beta and mean returns for plotting
    betas = betas.reshape(5, 5)
    mean_returns_matrix = mean_returns.values.reshape(5, 5)

    # Plotting
    plt.figure(figsize=(10, 8))
    
    # Loop to add text labels and different markers/colors for each row
    markers = ['o', 's', 'D', '^', 'v'] # Different markers for each size quintile
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd'] # Different colors
    
    # Plot scatter points for each portfolio
    for i in range(5):
        plt.scatter(betas[i, :], mean_returns_matrix[i, :], 
                    s=100, marker=markers[i], c=colors[i], label=f'Size {i+1} Portfolios')

    # Plot the cross-sectional regression line
    x_range = np.linspace(min(betas.flatten()), max(betas.flatten()), 100)
    y_line = cs_with_const_results['lambda_0'] + cs_with_const_results['lambdas'][0] * x_range
    plt.plot(x_range, y_line, 'r--', linewidth=2, label='Cross-Sectional Regression Line')

    # Set titles and labels
    plt.title(f'Mean Excess Returns vs. Beta ({model_name} Model)', fontsize=16)
    plt.xlabel('Beta (β̂)', fontsize=12)
    plt.ylabel('Average Monthly Excess Return (%)', fontsize=12)
    plt.grid(True, which='both', linestyle='--', linewidth=0.5)
    plt.legend(title='Portfolios (by Size)', loc='best')
    plt.tight_layout()
    plt.show()
